...
const AddHortalicaScreen = ({ route, navigation }: { route: Route; navigation: Navigation }) => {
  ...
  const loadHortalicas = async () => {
    try {
      ...
      Alert.alert('Erro', error?.message ?? 'Erro ao carregar hortaliças');
      navigation.goBack();
    } finally {
      setLoading(false);
    }
  ...
  const handleSelectHortalica = (id: number, nome: string) => {
  ...
  const handleCalculoDimensionamento = async () => {
    ...
    try {
      ...
      Alert.alert('Erro', error?.message ?? 'Erro ao calcular dimensionamento');
    } finally {
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      Alert.alert('Erro', error?.message ?? 'Erro ao salvar cultivo');
    } finally {
      setSaving(false);
    }
  };

  const renderHortalicaItem = ({ item }: { item: any }) => (
    <TouchableOpacity
